...
export async function getProductRating(id: number, isLoggedIn: boolean) {
  if (isLoggedIn) {
    return axios.get(API_BASE_URL + `/rating`, {
...
export async function createProductComment(
  ...
  return axios.post(
    ...
    { product: id, content: content, user: userId },
    options
  );
}
...
export async function giveProductRating(
  id: number,
  stars: number,
  existingRatingId: any
) {
  const options = {
    headers: { Authorization: `Bearer ${localStorage.getItem("access")}` },
  };
  if (existingRatingId) {
    return axios.put(
      API_BASE_URL + `/rating/${existingRatingId}/`,
...
export async function editCommentWithId(
  id: number,
  productId: number,
  content: string,
  userId: number
  ...
  return axios.put(
    ...
    { product: productId, content: content, user: userId },
    options
  );
}
